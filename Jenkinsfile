            ...
            steps{
                dir("${FRONTEND_DIR}") {
                    ...
                    withCredentials([usernamePassword(credentialsId: 'Docker_Hub_Credentials', usernameVariable: 'DOCKER_USER', passwordVariable: 'DOCKER_PASS')]) {
                        ...
                        sh 'docker tag frontend $DOCKER_USER/frontend:v7'
                        sh 'docker push $DOCKER_USER/frontend:v7'
                    }
                }
            }
        }
        stage('build backend'){
            steps{
                dir("${BACKEND_DIR}") {
                    ...
                    withCredentials([usernamePassword(credentialsId: 'Docker_Hub_Credentials', usernameVariable: 'DOCKER_USER', passwordVariable: 'DOCKER_PASS')]) {
                        sh 'docker login -u $DOCKER_USER -p $DOCKER_PASS'
                        sh 'docker tag backend $DOCKER_USER/backend:v7'
                        sh 'docker push $DOCKER_USER/backend:v7'
                    }
                }
            } 
        }    
